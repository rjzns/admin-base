...
Процедура РазмещениеКомандНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		бит_ДополнитьФильтрПоОбъектамМетаданных(Объект.Вид, ПараметрыФормы.ФильтрПоОбъектамМетаданных);	
		бит_ДополнитьВыбранныеОбъектыМетаданных(ПараметрыФормы);
...
// бит_Финанс изменения кода. Начало.
#Область бит_Финанс_Изменения

// бит_Финанс Функция определяет, какая команда используется в зависимости от вида дополнительной обработки.
//
// Параметры:
//   ВидОбработки - ПеречислениеСсылка.ВидыДополнительныхОтчетовИОбработок
//
// Возвращаемое значение:
//  ОбщаяКоманда – ОбъектМетаданных.
//
&НаСервереБезКонтекста
Функция бит_ОпределитьКомандуПоВидуОбработки(ВидОбработки)

	ОбщаяКоманда = Неопределено;
	Если ВидОбработки = Перечисления.ВидыДополнительныхОтчетовИОбработок.ЗаполнениеОбъекта Тогда
		ОбщаяКоманда = Метаданные.ОбщиеКоманды.бит_ДополнительныеОтчетыИОбработкиЗаполнениеОбъекта;
	ИначеЕсли ВидОбработки = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет Тогда
		ОбщаяКоманда = Метаданные.ОбщиеКоманды.бит_ДополнительныеОтчетыИОбработкиОтчеты;
	ИначеЕсли ВидОбработки = Перечисления.ВидыДополнительныхОтчетовИОбработок.СозданиеСвязанныхОбъектов Тогда
		ОбщаяКоманда = Метаданные.ОбщиеКоманды.бит_ДополнительныеОтчетыИОбработкиСозданиеСвязанныхОбъектов;
	КонецЕсли;
...
// бит_Финанс Дополняет фильтр-список объектов метаданных типами объектов команд БИТ. 
//
&НаСервереБезКонтекста
Процедура бит_ДополнитьФильтрПоОбъектамМетаданных(ВидОбработки, Фильтр)
	
	ОбщаяКоманда = бит_ОпределитьКомандуПоВидуОбработки(ВидОбработки);
	
	Если НЕ ОбщаяКоманда = Неопределено Тогда
		
		Для Каждого ТипПараметраКоманды Из ОбщаяКоманда.ТипПараметраКоманды.Типы() Цикл
			Фильтр.Добавить(Метаданные.НайтиПоТипу(ТипПараметраКоманды).ПолноеИмя());
		КонецЦикла;
...
// бит_Финанс Процедура дополняет коллекцию выбранных объектов метаданных объектами БИТ. 
//
&НаСервере
Процедура бит_ДополнитьВыбранныеОбъектыМетаданных(ПараметрыФормы)
	
	Для Каждого ЭлементНазначение Из Объект.Назначение Цикл
		Если Лев(ЭлементНазначение.ОбъектНазначения.Имя,4) = "бит_" 
			 И НЕ ПараметрыФормы.ФильтрПоОбъектамМетаданных.НайтиПоЗначению(ЭлементНазначение.ОбъектНазначения.ПолноеИмя) = Неопределено  Тогда
			
			ПараметрыФормы.ВыбранныеОбъектыМетаданных.Добавить(ЭлементНазначение.ОбъектНазначения.ПолноеИмя);
